...
import {
  cleanupAutoGeneratedCollectionsAndPapers,
...
async function cleanupAutoGeneratedContent({ accessToken = '' } = {}) {
  try {
    const payload = await cleanupAutoGeneratedCollectionsAndPapers({ accessToken });
    bookmarkIndexLoaded.value = false;
    return {
      cleaned: Boolean(payload?.cleaned),
      deletedCollections: Math.max(0, Number(payload?.deleted_collections || 0) || 0),
      deletedPapers: Math.max(0, Number(payload?.deleted_papers || 0) || 0),
      deletedNotes: Math.max(0, Number(payload?.deleted_notes || 0) || 0)
    };
  } catch {
    return {
      cleaned: false,
...
async function removeCollectionQuick(collectionId, { accessToken = '' } = {}) {
  const safeCollectionId = String(collectionId || '').trim();
  if (!safeCollectionId) return false;
  try {
    const payload = await removeCollection(safeCollectionId, { accessToken });
    if (!payload?.deleted) {
      return false;
    }
    await fetchCollections({ accessToken });
    ...
    collectionsErrorMessage.value = error?.message || '删除分组失败。';
    return false;
  }
}
